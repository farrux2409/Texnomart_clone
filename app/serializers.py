...
class ProductSerializer(serializers.ModelSerializer):
    ...
    def get_attributes(self, instance):
        attributes = instance.attributes.filter(product=instance).values_list('key_id', 'key__attribute_name',
    ...
    def get_all_images(self, instance):
        images = instance.product_images.filter(product=instance)
        all_images = []
    ...
    def get_primary_images(self, instance):
        image = instance.product_images.filter(is_primary=True).all()
        request = self.context.get('request')
        if image:
...
class ProductAttributeFilterSerializer(ModelSerializer):
    class Meta:
        model = ProductAttribute
